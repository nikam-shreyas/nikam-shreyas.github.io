...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case "volunteering":
        return <Volunteering onViewPost={handleVolunteeringSelect} />;
      case "volunteering-detail":
        const post = volunteeringData.find(
          (p) => p.id === selectedVolunteeringId
        );
        return post ? (
          <VolunteeringDetail
            post={post}
            onBack={() => handlePageChange("volunteering")}
          />
        ) : (
          <Volunteering onViewPost={handleVolunteeringSelect} />
        );
      default:
      ...
      >
        {currentPage !== "home" && (
          <div className="fixed top-6 left-6 md:left-10 z-50 animate-fade-in-up">
            <button
              onClick={() =>
                handlePageChange(
                  currentPage === "volunteering-detail"
                    ? "volunteering"
                    : "home"
                )
              }
        ...
        {currentPage === "home" && (
          <div className="w-full text-center pb-8 text-gray-600 text-xs">
            ©{new Date().getFullYear()} Crafted by Shreyas Nikam
          </div>
        )}
      </div>
    </main>
